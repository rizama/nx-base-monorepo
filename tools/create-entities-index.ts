...
function createEntitiesIndex() {
    console.log('Creating entity-index.ts for nest-typeorm app');
    const src = `${path.dirname(__dirname)}/apps/nest-typeorm/src`;

    if (!fs.existsSync(src)) {
        console.log(`App nest-typeorm cannot be found. Path not exist: ${src}`);
        process.exit(1);
    }
